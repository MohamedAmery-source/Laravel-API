...
3. لا يوجد أي شرط يفرض تطابق الجامعة في مسارات الدخول أو التقديم.
4. التقييد الأساسي يكون عبر صلاحيات الدور وحالة الحساب.
...
```
3. حقول مطلوبة فعليًا: `full_name`, `email`, `password`, `password_confirmation`, `user_type`.
...
5. عند أي `422`، اعرض السبب القادم من `message` أو تفاصيل الحقول.
